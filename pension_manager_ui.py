...
def _auto_price(ticker):
    ...
    try:
        ...
        close = hist["Close"]
        # yfinance 버전에 따라 단일 티커도 DataFrame으로 반환될 수 있다.
...
def render_pension_manager_tab():
    ...
    st.caption(
        "월급 후 매월 정액 매수합니다. 단기 주가 변동으로 적립 시점을 바꾸지 않고, "
        "현재가는 자동 갱신하며 MA5는 보유자산의 위험관리 참고 신호로만 사용합니다."
    )
    ...
    with c1:
        ...
        if korea_auto is not None:
            st.metric("자동 현재가", _won(korea_auto))
            st.caption(f"기준일 {korea_asof} · {korea_ticker}")
            korea_price = korea_auto
        else:
            st.error("현재가 자동조회 실패")
            korea_price = st.number_input("KOREA TOP10 수동 현재가", min_value=0, step=100, value=0, format="%d")
    ...
    with c2:
        ...
        if sp_auto is not None:
            st.metric("자동 현재가", _won(sp_auto))
            st.caption(f"기준일 {sp_asof} · {sp_ticker}")
            sp_price = sp_auto
        else:
            st.error("현재가 자동조회 실패")
            sp_price = st.number_input("S&P500 ETF 수동 현재가", min_value=0, step=100, value=0, format="%d")

    with c3:
        st.markdown("**안전자산 / 신호**")
        safe_now = st.number_input("채권·현금성 평가액", min_value=0, step=100000, value=0, format="%d")
        korea_signal = st.selectbox(
            "KOREA TOP10 MA5 참고신호",
            [
                "🟢 MA5 위 · 상승",
                "🟡 MA5 위 · 횡보",
                "🟠 MA5 부근",
    ...
    display_rows.append(
        {
            ...
            "수익률": "-",
            "목표비중": "20%",
            "현재비중": f"{(safe_now / invested_total * 100 if invested_total else 0):.1f}%",
        }
    )
    ...
    p2.metric("주식 수익금", _won(stock_profit))
    stock_cost = korea_qty * korea_avg + sp_qty * sp_avg
    p3.metric(
        "주식 수익률",
        f"{(((korea['value'] + sp['value']) / stock_cost - 1) * 100 if stock_cost else 0):+.2f}%",
    )

    st.markdown("### 🎯 장기 목표비중")
    st.write("S&P500 **50%** · KOREA TOP10 **30%** · 채권·현금성 **20%**")

    # 월 납입금은 시장 타이밍과 무관하게 반드시 투자한다.
    # 현재 비중이 목표에서 벗어나면 매도보다 신규 납입금을 부족 자산에 우선 배분한다.
    target_values = {
        "sp": (invested_total + monthly) * 0.50,
        "korea": (invested_total + monthly) * 0.30,
        "safe": (invested_total + monthly) * 0.20,
    }
    gaps = {
        "sp": max(0, target_values["sp"] - sp["value"]),
        "korea": max(0, target_values["korea"] - korea["value"]),
        "safe": max(0, target_values["safe"] - safe_now),
    }
    ...
    st.markdown("### 📅 운용 원칙")
    st.write(
        "① 월급 후 월 50만원 정기매수 ② 단기 주가 변동으로 매수일 변경하지 않음 "
        "③ 매도보다 신규 납입금으로 50:30:20 목표비중 조정 "
        "④ MA5는 정기적립 중단 신호가 아니라 기존 보유자산의 위험관리 참고 신호"
    )

    with st.expander("내년 퇴직 후 IRP 준비"):
        st.write(
            "퇴직금이 IRP로 들어오면 동일한 보유수량·평균단가·자동 현재가·수익률 구조를 IRP에도 추가하고, "
            "연금저축+IRP 합산 목표비중과 월간 실행금액을 관리합니다."
        )
