...
from korea_backtest_ui import render_backtest_tab
...
FINAL_TOP_N=12;DEEP_CANDIDATE_COUNT=120;YF_CHUNK=180;MIN_PRICE=1000;MIN_AVG_VALUE=2_000_000_000
FALLBACK_UNIVERSE=[("005930","삼성전자","KOSPI"),("000660","SK하이닉스","KOSPI"),("035420","NAVER","KOSPI"),("035720","카카오","KOSPI"),("005380","현대차","KOSPI"),("000270","기아","KOSPI"),("207940","삼성바이오로직스","KOSPI"),("068270","셀트리온","KOSPI"),("373220","LG에너지솔루션","KOSPI"),("006400","삼성SDI","KOSPI"),("005490","POSCO홀딩스","KOSPI"),("051910","LG화학","KOSPI"),("012450","한화에어로스페이스","KOSPI"),("042660","한화오션","KOSPI"),("009540","HD한국조선해양","KOSPI"),("034020","두산에너빌리티","KOSPI"),("105560","KB금융","KOSPI"),("055550","신한지주","KOSPI"),("086790","하나금융지주","KOSPI"),("316140","우리금융지주","KOSPI"),("028260","삼성물산","KOSPI"),("066570","LG전자","KOSPI"),("003670","포스코퓨처엠","KOSPI"),("323410","카카오뱅크","KOSPI"),("247540","에코프로비엠","KOSDAQ"),("086520","에코프로","KOSDAQ"),("196170","알테오젠","KOSDAQ"),("028300","HLB","KOSDAQ"),("058470","리노공업","KOSDAQ"),("403870","HPSP","KOSDAQ"),("214150","클래시스","KOSDAQ"),("039030","이오테크닉스","KOSDAQ")]

def save_json(path,obj):path.write_text(json.dumps(obj,ensure_ascii=False,indent=2),encoding="utf-8")
def market_open():
    now=datetime.now(SEOUL);return now.weekday()<5 and time(9,0)<=now.time()<=time(15,30)
def clip(x,lo,hi):return float(np.clip(float(x),lo,hi))
...
def load_csv_universe():
    ...
    try:
        df=pd.read_csv(UNIVERSE_FILE,dtype={"종목코드":str});df["종목코드"]=df["종목코드"].astype(str).str.zfill(6);return df[["종목코드","종목명","시장"]].copy()
...
def build_market_screen(universe,flow_map,progress=None):
    ...
    if flow_present:
        df["외국인백분위"]=df["외국인순매수"].rank(pct=True)*100;df["기관백분위"]=df["기관순매수"].rank(pct=True)*100;df["1차점수"]=df["유동성백분위"]*.25+df["당일강도백분위"]*.15+df["20일강도백분위"]*.15+df["외국인백분위"]*.225+df["기관백분위"]*.225
    else:df["외국인백분위"]=50.;df["기관백분위"]=50.;df["1차점수"]=df["유동성백분위"]*.40+df["당일강도백분위"]*.25+df["20일강도백분위"]*.35
...
def deep_analyze(screen):
    candidates=screen.head(max(DEEP_CANDIDATE_COUNT,FINAL_TOP_N*4)).copy();symbols=[yf_symbol(r["종목코드"],r["시장"]) for _,r in candidates.iterrows()];batch=download_chunk(tuple(symbols),"6mo");rows=[];flow_present=(screen["외국인순매수"].abs().sum()+screen["기관순매수"].abs().sum())>0
    for _,r in candidates.iterrows():
        code=str(r["종목코드"]).zfill(6);h=extract_one(batch,yf_symbol(code,r["시장"]))
        if len(h)<60:h=get_single_history(yf_symbol(code,r["시장"]),"6mo")
        if len(h)<60:continue
        close=pd.to_numeric(h["Close"],errors="coerce").dropna();vol=pd.to_numeric(h["Volume"],errors="coerce").dropna()
        if len(close)<60:continue
        p=float(close.iloc[-1]);ma20=float(close.tail(20).mean());ma60=float(close.tail(60).mean());high20=float(close.tail(20).max());vol_ratio=float(vol.tail(5).mean()/max(vol.tail(20).mean(),1));trend=100 if p>ma20>ma60 else (72 if p>ma60 else 40);momentum=clip(50+(p/float(close.iloc[-21])-1)*180,0,100);volume_score=clip(50+(vol_ratio-1)*35,0,100);flow_score=50.
        if flow_present:flow_score=clip((float(r["외국인백분위"])+float(r["기관백분위"]))/2,0,100)
        fundamental=fundamental_score(code,r["시장"]);technical=trend*.45+momentum*.35+volume_score*.20;score=technical*.40+float(r["유동성백분위"])*.12+fundamental*.23+flow_score*.25;overheat=p/high20>=.985 and momentum>=75;buy1=min(p,ma20*1.01);buy2=min(buy1,ma60*1.015 if ma60>0 else buy1*.97);stop=buy2*.97
        rows.append({"_종목코드":code,"_시장":r["시장"],"종목명":r["종목명"],"현재가":round(p),"KOREA점수":round(score,1),"종합점수":round(score,1),"판정점수":round(score,1),"기술점수":round(technical,1),"펀더멘털":round(fundamental,1),"수급점수":round(flow_score,1),"과열":"과열" if overheat else "정상","1차 매수가":round(buy1,-2),"2차 매수가":round(buy2,-2),"손절가(3%)":round(stop,-2),"수급실데이터":flow_present,"수급·질적 종합의견":f"외국인 {r['외국인순매수']:,.0f} / 기관 {r['기관순매수']:,.0f} · 펀더멘털 {fundamental:.0f}"})
    return sorted(rows,key=lambda x:x["종합점수"],reverse=True)
def kospi_regime():
    h=yf_history("^KS11","1y")
    if len(h)<120:return "중립장"
    c=pd.to_numeric(h["Close"],errors="coerce").dropna();p=float(c.iloc[-1]);m20=float(c.tail(20).mean());m60=float(c.tail(60).mean());m120=float(c.tail(120).mean())
    if p>m20>m60>m120:return "강세장"
    if p<m20<m60:return "약세장"
    return "중립장"
def apply_relative(rows,regime):
    if not rows:return rows,75,10
    floor,active_pct=(72,12) if regime=="강세장" else ((75,10) if regime=="중립장" else (80,7));n=len(rows);flow_real=any(r.get("수급실데이터") for r in rows)
    for i,r in enumerate(rows):
        pct=(i+1)/n*100;r["상대순위"]=f"상위 {pct:.1f}%";r["시장상태"]=regime
        if not flow_real:r["판정"]="🔵 관찰" if r["종합점수"]>=floor-7 else "⚪ 현금대기";r["수급대응"]="수급대기"
        elif r["종합점수"]>=floor and pct<=active_pct and r["과열"]!="과열":r["판정"]="🟢 적극매수";r["수급대응"]="확인"
        elif r["종합점수"]>=floor-7:r["판정"]="🟡 매수후보";r["수급대응"]="대기"
        elif r["종합점수"]>=floor-15:r["판정"]="🔵 관찰";r["수급대응"]="대기"
        else:r["판정"]="⚪ 현금대기";r["수급대응"]="대기"
        if r["과열"]=="과열" and r["판정"].startswith("🟢"):r["판정"]="🟡 매수후보"
...
tabs=st.tabs(["🌐 시장환경","🔎 전체시장 분석","🏆 TOP12","📊 백테스트","🔔 카카오 준비","⚙️ 설정"])
with tabs[0]:
    regime=kospi_regime();c1,c2=st.columns(2);c1.metric("현재 시장 레짐",regime);c2.metric("한국 정규장","OPEN" if market_open() else "CLOSED","09:00~15:30 KST");st.subheader("KOSPI vs 한국 수출 YoY");kd=yf_history("^KS11","5y");exp=load_export_history()
    if not kd.empty:
        ...
        if not exp.empty:e=exp.set_index("date");use=[c for c in ["export_yoy","semi_yoy"] if c in e.columns];chart=m[["KOSPI YoY"]].join(e[use],how="outer").sort_index();chart=chart.rename(columns={"export_yoy":"수출 YoY","semi_yoy":"반도체 수출 YoY"});st.line_chart(chart)
...
with tabs[2]:
    ...
    if not rows:st.info("먼저 '전체시장 분석'에서 자동분석을 실행하세요.")
        ...
        top=rows[:FINAL_TOP_N];display=pd.DataFrame([{"순위":i,"종목":r["종목명"],"현재가(원)":r["현재가"],"판정점수":r["판정점수"],"시장상태":r["시장상태"],"상대순위":r["상대순위"],"수급대응":r["수급대응"],"과열":r["과열"],"판정":r["판정"],"1차매수가(원)":r["1차 매수가"],"2차매수가(원)":r["2차 매수가"],"손절가(3%)(원)":r["손절가(3%)"]} for i,r in enumerate(top,1)]);st.dataframe(display,use_container_width=True,hide_index=True);st.caption("1차/2차 매수가는 20일선·60일선과 눌림목을 반영한 적극매수 가격대입니다. 손절가는 2차 매수가 대비 -3%입니다.");st.markdown("## 최종 3종목 후보");active=[r for r in top if str(r["판정"]).startswith("🟢 적극매수")]
...
with tabs[5]:st.markdown("""### V3.9 핵심
- TOP12 / KOSPI + KOSDAQ 전체시장
- yfinance 가격·거래량 스크리닝
- KRX 투자자 순매수 독립 수집
- 실제 수급 미확보 시 적극매수 잠금
- 💼 보유종목 영구저장/추가매수 평균단가/전량매도 관리
- 📊 최근 3년 기본 백테스트 / CAGR / MDD / 승률 / KOSPI 비교
""")