...
def get_market_cap_ranking(page_size=1000):
    """Naver current market-cap ranking. Used only when KRX blocks server requests."""
    try:
        data = _get_json(
...
def _find_investor_value(d, kind):
    kind_tokens = ("foreign", "foreigner", "frgn", "외국") if kind == "foreign" else ("institution", "organization", "org", "기관")
    preferred = []
    fallback = []
    for k, v in d.items():
        lk = str(k).lower()
        if not any(t in lk for t in kind_tokens):
            continue
        n = _num(v)
        if n is None:
            continue
...
def get_stock_flow(code):
    """Latest Naver investor-trend row for one stock. Returns foreign/institution net values when available."""
    try:
        data = _get_json(
            f"/api/domestic/detail/{code}/trend",
            {"tradeType": "KRX", "startIdx": 0, "pageSize": 5},
        )
    except Exception:
        return None
