...
def get_market_cap_data():
    """KRX를 우선 사용하고, 실패하면 Naver 현재 시총순위를 대체 데이터로 사용한다."""
    ...
    if not now_df.empty:
        ...
        return out, {"source": "KRX", "date": now_date, "full_momentum": True}
    ...
    if not nv.empty:
        ...
        return out, {"source": "NAVER 현재순위(대체)", "date": nv_date, "full_momentum": False}

    return pd.DataFrame(), {"source": "없음", "date": None, "full_momentum": False}
...
def jump_grade(score, cap_score, flow_real, overheat, regime):
    if not flow_real or score is None or cap_score is None:
        return "⚪ 데이터대기"
...
def render_wealth_jump_tab(rows, regime="중립장", analysis_at=None):
    ...
    st.caption("KRX를 우선 사용하고, KRX가 막히면 네이버 금융 읽기 전용 데이터로 자동 보완합니다.")
    if not rows:
    ...
    if cap_ok and not cap_meta.get("full_momentum"):
    ...
    for row in rows:
        ...
        x["시총모멘텀"] = cap_score
        x["수급점수"] = live_flow
        x["수급실데이터"] = live_flow is not None
        x["Conviction"] = wealth_jump_score(x, cap_score, live_flow)
        x["점프등급"] = jump_grade(x["Conviction"], cap_score, x["수급실데이터"], x.get("과열", "정상"), regime)
    ...
    display = []
    for i, r in enumerate(top, 1):
        display.append({"순위": i, "종목": r.get("종목명"), "Conviction": _score_text(r.get("Conviction")), "등급": r.get("점프등급"), "현재 시총순위": _rank_text(r.get("현재순위")), "4주 변화": "-" if r.get("4주순위변화") is None or pd.isna(r.get("4주순위변화")) else f"{float(r.get('4주순위변화')):+.0f}", "12주 변화": "-" if r.get("12주순위변화") is None or pd.isna(r.get("12주순위변화")) else f"{float(r.get('12주순위변화')):+.0f}", "시총모멘텀": _score_text(r.get("시총모멘텀")), "기술": _score_text(r.get("기술점수")), "펀더멘털": _score_text(r.get("펀더멘털")), "수급": _score_text(r.get("수급점수")), "과열": r.get("과열")})
    ...
    if not cap_ok:
        ...
        cols = [c for c in ["종목명", "현재순위", "4주전순위", "4주순위변화", "12주전순위", "12주순위변화", "시총모멘텀점수"] if c in radar.columns]
    ...
    st.info(r["실행근거"])
    st.caption("KRX 실패 시 네이버 금융 읽기 전용 API를 보조 소스로 사용합니다. 네이버 시총 대체값은 현재 순위 기반이라 4주·12주 순위변화보다 보수적으로 해석하세요.")
